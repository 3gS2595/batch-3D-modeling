...
public class GenerateThreaded {
    ...
    public static Form gen(Form[] parent) {
        GenerateThreaded run = new GenerateThreaded();
        parent[0].v = run.run(parent);
        return parent[0];
    }
    public List<double[]> run(Form[] parent){
        cnt = -1;
        this.synlist = Collections.synchronizedList(new ArrayList<>(parent[0].v));
    ...
    public void generate(int i, Form[] parent, List<double[]> synl, ProgressBar pb0) {
